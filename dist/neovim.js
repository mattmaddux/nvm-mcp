 ...
 */
export function findNeovimSockets() {
    const tmpDir = "/tmp";
    const sockets = [];
 ...
 */
export function getNeovimInstance(pid) {
 ...
 */
export async function getNeovimInstanceDetails(pid) {
    ...
    try {
        ...
        try {
            ...
            const [line, column] = await nvim.call('getpos', '.').then((pos) => [pos[1], pos[2]]);
            // Get all buffers
            const allBuffers = await nvim.buffers;
            const buffers = await Promise.all(allBuffers.map(async (buffer, index) => {
 ...
 */
export function formatNeovimInstances(instances) {
 ...
 */
export function formatNeovimInstanceDetails(details) {
 ...
 */
export async function openFileInNeovim(pid, filePath, line, column, endLine, endColumn) {
    const instance = getNeovimInstance(pid);
    if (!instance) {
        return {
            success: false,
 ...
 */
export async function executeInNeovim(pid, command, isKeySequence = false) {
    const instance = getNeovimInstance(pid);
    if (!instance) {
        return {
            success: false,
