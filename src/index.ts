...
import {
  findNeovimSockets,
  getNeovimInstanceDetails,
  formatNeovimInstances,
  formatNeovimInstanceDetails,
  openFileInNeovim,
  executeInNeovim,
  NeovimInstanceDetails,
  NeovimOpenFileResult,
  NeovimExecuteResult,
} from "./neovim.js";
...
server.tool(
  ...
  "List all currently running Neovim instances",
  async () => {
    try {
...
server.tool(
  "get-neovim-instance",
  "Get detailed information about a specific Neovim instance including working directory, buffers, and cursor position",
  {
...
server.tool(
  "neovim-open-file",
  "Open/switch to a file in Neovim, optionally jump to line/column and select a region",
  {
    pid: z.number().describe("Process ID of the Neovim instance"),
...
server.tool(
  "neovim-execute",
  "Execute a Vim command or key sequence in Neovim (replaces nvmctl exec)",
  {
    pid: z.number().describe("Process ID of the Neovim instance"),
